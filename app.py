...
def payment():
    if request.method == 'POST' and request.files:
        ...
        test_image_array = glob.glob(os.path.join(
            app.config["IMAGE_UPLOADS"], imagex.filename))
...
def login():
    if request.method == 'POST' and request.files:
        ...
        face_client = FaceClient(ENDPOINT, CognitiveServicesCredentials(KEY))
        test_image_array = glob.glob(os.path.join(
            app.config["LOGIN"], imagex.filename))
        image = open(test_image_array[0], 'r+b')
...
def getinfo():
    if request.method == 'POST' and request.files:
        ...
        face_client = FaceClient(ENDPOINT, CognitiveServicesCredentials(KEY))
        test_image_array = glob.glob(os.path.join(
            app.config["IMG_MEDICINES"], imagex.filename))
        image = open(test_image_array[0], 'r+b')
